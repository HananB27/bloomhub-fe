...
const DEFAULT_RETURN_TO = "/?module=time-tracking&jira_connected=1";
...
function readReturnTo(): string {
  try {
    ...
    if (stored && stored.startsWith("/")) {
      ...
      return `${stored}${sep}jira_connected=1`;
    }
  } catch {
...
function JiraOAuthCallbackInner() {
  ...
  const retry = useCallback(async () => {
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Failed to restart Jira OAuth";
      notifyError(message);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await timeTrackingApi.completeJiraOAuth(code, stateParam);
        notifySuccess("Jira connected");
        setState({ kind: "success" });
        router.replace(readReturnTo());
      } catch (e) {
        const message =
          e instanceof Error ? e.message : "Failed to complete Jira OAuth";
        notifyError(message);
        setState({ kind: "error", message });
      }
    })();
        ...
        <CardContent className="space-y-4 px-5 py-6">
          {state.kind === "pending" && (
            <div className="flex items-center gap-3 text-sm text-gray-700">
              ...
              Finalizing your Jira connection…
            </div>
          )}
          {state.kind === "success" && (
            <div className="flex items-center gap-3 text-sm text-gray-700">
